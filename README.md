...
| `index.html` | **De nieuwe website (18 aug 2026).** Alle 9 routes in één design: vergelijking, overzichtskaart, detail per route, mediaknoppen per stop, bewerkmodus. Dubbelklikken werkt (kaarttegels hebben internet nodig). |
...
| `samenvatting-camperreis-2027.html` | Samenvatting camperonderzoek (17 aug 2026): twee finalisten, vertrekmoment, reserveringskalender. |
| `route-6b-light.kml` / `route-4-plus.kml` | Google My Maps-exports van de twee finalisten (exacte stops + coördinaten). |
| `camper-vergelijking.html` | **Oude site (generatie 1)** — routetekeningen kloppen niet meer met de Google Maps-knoppen. Archief-kandidaat. |
| `routekaart-6b-light.html` / `routekaart-route-4-plus.html` | **Oude detailpagina's (generatie 2)** — inhoud klopt, design verouderd. Archief-kandidaat. |
| `camper-vergelijking-SAMENVATTING.md` | Leeswijzer bij de oude vergelijkingspagina. |
| `og-card.png` / `og-card.svg` | WhatsApp-preview (nog oud ontwerp — vernieuwen vóór de volgende push). |
...
- **`index.html` is lokaal de nieuwe site; GitHub Pages toont nog de oude** tot de volgende push. De oude pagina's (`camper-vergelijking.html`, `routekaart-*.html`) zijn archief-kandidaten — hun inhoud is opgenomen in de nieuwe site.
- **De oude site had een verouderde routetekening** (o.a. een Colorado/Utah-lus mét Arches NP — daar zijn jullie al geweest). De nieuwe site tekent exact de Google Maps-routes; de 19 bezochte plekken staan als filter in `routes-data.js` (`node check-routes.js` waarschuwt als er ooit één in een route opduikt).
- **og-card nog oud** ("Zes routes, één camper") — vernieuwen voor de push.
